...
// GPLv3, unlike most of mpv: this AO builds against Steinberg's ASIO SDK
// headers, used under the GPLv3 half of the SDK's dual licence. See the note
// at the top of ao_asio.h and the GPL-only file list in Copyright.
                             ...
                             struct mp_log *log)
{
    ...
    for (DWORD i = 0; n < max; i++) {
        ...
        DWORD clen = sizeof(clsid_str);
        DWORD type = 0;
        if (RegQueryValueExW(sub, L"CLSID", NULL, &type, (BYTE *)clsid_str,
                             &clen) == ERROR_SUCCESS && type == REG_SZ)
        {
            clsid_str[(clen / sizeof(wchar_t)) - 1] = 0;
            CLSID clsid;
...
static void asio_teardown(struct ao *ao)
{
    ...
    if (s->iasio) {
        if (s->buffers_created) {
            ...
            // safer; ignore errors.
            s->iasio->lpVtbl->stop(s->iasio);
            s->iasio->lpVtbl->disposeBuffers(s->iasio);
